...
EOF
        ...
        if [[ "${USE_WIREGUARD:-n}" == "y" ]]; then
            ...
            # --- STRICT ZERO TRUST HIERARCHY (v2.23) - DEBIAN PARITY) ---
...
uninstall_syswarden() {
    ...
    # --- Clean up all SysWarden Fail2ban filters (Including v2.23 additions) ---
...
# ==============================================================================
# SYSWARDEN v2.23 - TELEMETRY BACKEND
# ==============================================================================
                ...
                for IP in $BANNED_IPS; do
                    if [[ -n "$IP" ]]; then
                        ...
                        if [[ "$JAIL" =~ (recidive) ]]; then
                            ...
                            # Web Jails: Check Nginx and Apache access/error logs (-h suppresses filename output)
